...
ABICoder.prototype.encodeParameters = function (types, params) {
    ...
    if(utils.isObject(types) && types.inputs) {
        types = (types.inputs||[]).map(function (input) {
            return input.type;
        });
    }
...
ABICoder.prototype.decodeParameters = function (outputs, bytes) {
    var isTypeArray = utils.isArray(outputs) && utils.isString(outputs[0]);
...
ABICoder.prototype.decodeLog = function (inputs, data, topics) {
    ...
    var indexedData = utils.isArray(topics) ? topics.map(function (topic) { return topic.slice(2); }).join('') : topics;
